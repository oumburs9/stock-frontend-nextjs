...
import { PayablesAgingChart } from "@/components/reports/charts/payables-aging-chart"
import { usePayablesReport, useExportPayables } from "@/lib/hooks/use-reports"
import { formatCurrency, formatNumber } from "@/lib/utils/export"
import { formatDateShort } from "@/lib/utils/date"
import type { PayablesFilters } from "@/lib/types/report"

export default function PayablesPage() {
  const [filters, setFilters] = useState<PayablesFilters>({})
  const { data, isLoading, refetch } = usePayablesReport(filters)
  const exportMutation = useExportPayables()

  const totalPayables = data?.length || 0
  ...
  return (
    <DashboardLayout>
      <ReportPageLayout
        ...
        description="Track outstanding payables and payment aging"
        filters={null}
        actions={
          <ExportButton
        ...
        kpis={
          <>
            <KpiCard title="Total Payables" value={formatNumber(totalPayables)} icon={Receipt} />
      ...
      >
        ...
        {data && data.length > 0 ? (
          ...
          <EmptyState title="No payables data available" description="No payables found" />
        )}
      </ReportPageLayout>
    </DashboardLayout>
  )
